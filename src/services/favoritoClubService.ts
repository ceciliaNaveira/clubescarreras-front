...
/** Listar todos los favoritos (solo para admin o debug) */
...
/** Obtener los favoritos de un club (quiénes lo marcaron) */
...
export const addFavoritoClub = async (usuarioId: number, clubId: number): Promise<FavoritoClub> => {
  ...
  if (!res.ok) {
    if (res.status === 409) {
      // Ya existe en favoritos, devolvemos un objeto ficticio para no romper toggle
